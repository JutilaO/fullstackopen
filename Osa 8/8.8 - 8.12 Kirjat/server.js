...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
        let list = books
        if(args.name) list = books.filter(book => book.author === args.name)
        if(args.genre) list = list.filter(book => book.genres.includes(args.genre))
        return list
    },
    allAuthors: () => {
        let temporary = [...authors]
        for(let i in temporary){
            temporary[i].bookCount = books.filter(book => book.author === temporary[i].name).length
        }
        return temporary
    }
  },
